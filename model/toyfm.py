...
class VelocityField(eqx.Module):
    ...
    def __call__(
        ...
        packed, _ = pack([t, x], "B *")
        h = jax.nn.relu(self.in_layer(packed))
...
def train_on(
    ...
    def make_update(key: PRNGKeyArray, model: ToyFM, batch: jax.Array, optimizer_state):
        ...
        loss, grad = jax.value_and_grad(model.train_step)(t, rand_input, batch)
    ...
    for epoch in range(n_epochs):
        for batch in dataset:
            key, model, optimizer_state, _loss = make_update(
                key,
                model,
                batch,
                optimizer_state,
            )
            # TODO(nikhil): Log loss
