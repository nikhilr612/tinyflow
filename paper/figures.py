...
REGION_COLORS = {"face": "#c9b58e", "eyes": "#5aa9c9", "mouth": "#d46a6a", "hair/bg": "#9c9c9c"}
...
def box(ax, xy, w, h, text="", fc=LIGHT, ec=INK, lw=0.7, fs=8, r=0.06, color=INK, **kw):
    ...
    x, y = xy
    ax.add_patch(
        FancyBboxPatch((x, y), w, h, boxstyle=f"round,pad=0,rounding_size={r}", fc=fc, ec=ec, lw=lw, **kw)
    )
    if text:
...
def arrow(ax, p, q, color=INK, lw=0.7, style="-|>", rad=0.0, ls="-", ms=6, shrink=0):
    ...
    a = FancyArrowPatch(
        p, q, arrowstyle=style, mutation_scale=ms, color=color, lw=lw, ls=ls,
...
def architecture():
    ...
    for i, ((res, ch), y) in enumerate(zip(levels, ys)):
        enc.append(box(ax, (enc_x, y), bw, bh, f"ResBlock\n{ch}$\\to${2 * ch}", fc="#e3ebf5", ec=BLUE, fs=6.2))
        dec.append(box(ax, (dec_x, y), bw, bh, f"ResBlock\n{2 * ch}$\\to${ch}", fc="#f8e8dc", ec=ORANGE, fs=6.2))
        ax.text(enc_x - 0.04, y + bh + 0.02, f"{res}$^2$", ha="right", va="bottom", fontsize=6.5, color=GREY)
    # encoder chain (strided depthwise conv between levels)
    for i in range(3):
        arrow(ax, (enc[i][0], ys[i]), (enc[i][0], ys[i + 1] + bh), color=BLUE)
        ax.text(enc[i][0] + 0.05, (ys[i] + ys[i + 1] + bh) / 2, "2$\\times$2 stride-2\ndepthwise", ha="left", va="center", fontsize=5, color=GREY)
    # 4x4 tensor, no mid block
    tx0 = (enc[3][0] + dec[3][0]) / 2
    ax.text(tx0, 0.40, "$4^2\\times1024$ (no mid block)", ha="center", va="center", fontsize=6, color=GREY)
    ...
    for i in range(3, 0, -1):
        ...
        if i in (2, 1):
            ...
            box(ax, (dec_x + 0.05, rp_y), bw - 0.10, 0.16, "RegionPool", fc="#e2f0e7", ec=GREEN, fs=5.8, color=GREEN)
        ...
        ax.text(dec[i][0] + bw / 2 + 0.05, (y_from + y_to) / 2, "$\\uparrow$2 bilinear\n+ 3$\\times$3 conv", ha="left", va="center", fontsize=5, color=GREY)
    ...
    for i in range(4):
        ...
        src = (enc[i][0] + bw / 2 + 0.03, y_s) if i > 0 else (enc[0][0] + bw / 2 + 0.03, y_s)
        ...
        ax.text(dst[0] - 0.02, y_s + 0.03, lab, ha="right", va="bottom", fontsize=5.2, color=GREY)
    # inputs
    ix = 0.08
    box(ax, (ix, ys[0] + 0.03), 0.92, 0.24, "in-conv 3$\\times$3 $\\to$ 64", fs=5.8)
    box(ax, (ix, ys[0] - 0.48), 0.92, 0.36, "$[x_t,\\ m]$\n$64^2\\times$(3+3)", fc="#f7e3e2", ec=RED, fs=6)
    arrow(ax, (ix + 0.46, ys[0] - 0.12), (ix + 0.46, ys[0] + 0.03))
    arrow(ax, (ix + 0.92, ys[0] + bh / 2), (enc_x, ys[0] + bh / 2))
    ax.text(ix + 0.46, ys[0] - 0.53, "image, layout masks", ha="center", va="top", fontsize=5.5, color=RED)
    # outputs
    ox = 4.62
    box(ax, (ox, ys[0] + 0.03), 0.90, 0.24, "out-conv 1$\\times$1 $\\to$ 3", fs=5.8)
    box(ax, (ox, ys[0] - 0.48), 0.90, 0.36, "$\\hat{x}$  (no output\nactivation)", fc="#f8e8dc", ec=ORANGE, fs=6)
    arrow(ax, (dec_x + bw, ys[0] + bh / 2), (ox, ys[0] + bh / 2), color=ORANGE)
    arrow(ax, (ox + 0.45, ys[0] + 0.03), (ox + 0.45, ys[0] - 0.12), color=ORANGE)
    ax.text(ox + 0.45, ys[0] - 0.53, "$v=\\dfrac{\\hat{x}-x_t}{\\max(1-t,\\,0.05)}$", ha="center", va="top", fontsize=6.5)
    ...
    box(ax, (rail - 0.60, ty), 1.20, 0.24, "sinusoid($10^3\\,t$) $\\to$ MLP $=e(t)$", fc="#ece6f4", ec=PURPLE, fs=5.8)
    arrow(ax, (rail - 0.69, ty + 0.12), (rail - 0.60, ty + 0.12), color=PURPLE)
    ax.text(rail + 0.68, ty + 0.12, "each adaLN: its own zero-init\nlinear $e(t)\\mapsto(1+\\gamma,\\beta)$", ha="left", va="center", fontsize=5.4, color=PURPLE)
    ax.plot([rail, rail], [ty, ys[3] + bh / 2], color=PURPLE, lw=0.8)
    nw, nh = 0.30, 0.15
    for i, y in enumerate(ys):
        yc = y + bh / 2
        for x_node, x_block in ((enc_x + bw + 0.06, enc_x + bw), (dec_x - 0.06 - nw, dec_x)):
            box(ax, (x_node, yc - nh / 2), nw, nh, "adaLN", fc="#ece6f4", ec=PURPLE, fs=5, color=PURPLE, lw=0.5)
    ...
    ax.text(enc_x - 0.08, ys[0] + bh + 0.16, "encoder", ha="right", va="bottom", fontsize=7, color=BLUE)
    ax.text(dec_x + bw + 0.08, ys[0] + bh + 0.16, "decoder", ha="left", va="bottom", fontsize=7, color=ORANGE)
    ax.text(2.8, 0.12, "37 M parameters, GroupNorm (8 groups) throughout.  The RegionPool layers read the masks $m$ straight from the input.",
...
def _mask_icon(ax, x, y, s, which, color):
    ...
    if which == "hair/bg":
        ...
        for dx in (-0.16, 0.16):
            ax.add_patch(Ellipse((x + s * (0.5 + dx), y + s * 0.58), s * 0.16, s * 0.10, fc=color, ec="none"))
    else:  # mouth
        ax.add_patch(Ellipse((x + s / 2, y + s * 0.27), s * 0.16, s * 0.06, fc=color, ec="none"))
...
def regionpool():
    ...
    def fmap(x, y, s, label):
        for k in range(3):
            ax.add_patch(Polygon([(x + 0.05 * k, y + 0.05 * k), (x + s + 0.05 * k, y + 0.05 * k),
                                  (x + s + 0.05 * k, y + s + 0.05 * k), (x + 0.05 * k, y + s + 0.05 * k)],
                                 closed=True, fc="#dde6f1", ec=INK, lw=0.5))
    ...
    ax.text(mx + 0.47, my - 0.05, "$m_k$: face, eyes, mouth, hair/bg", ha="center", va="top", fontsize=5.4)
    ...
    ax.text(1.38, mid + 0.05, "masked mean", ha="center", va="bottom", fontsize=6, color=GREEN)
    ax.text(1.52, mid - 0.07, "$\\bar e_k=\\dfrac{\\sum_p m_k(p)\\,h(p)}{\\sum_p m_k(p)}$", ha="center", va="top", fontsize=6.2)
    plus = (4.12, mid)
    for j, (name, c) in enumerate(REGION_COLORS.items()):
        y = mid + 0.48 - 0.32 * j - 0.085
        ax.add_patch(FancyBboxPatch((px, y), 0.46, 0.17, boxstyle="round,pad=0,rounding_size=0.02", fc=c, ec=INK, lw=0.4))
        r = name.split("/")[0]
        ax.text(px + 0.23, y + 0.085, "$\\bar e_{\\mathrm{%s}}$" % r, ha="center", va="center", fontsize=6)
        box(ax, (px + 0.72, y - 0.01), 0.80, 0.19, "$W_{\\mathrm{%s}}\\bar e_{\\mathrm{%s}}+b_{\\mathrm{%s}}$" % (r, r, r), fc=LIGHT, ec=GREEN, fs=5.6)
    ...
    ax.text(plus[0] + 0.05, mid - 0.16, "$\\sum_k m_k\\otimes(W_k\\bar e_k+b_k)$\nbroadcast into region", ha="center", va="top", fontsize=5.4)
    ...
    top = mid + 0.62
    ax.plot([0.55, 0.55, plus[0], plus[0]], [mid + 0.37, top, top, plus[1] + 0.09], color=GREY, lw=0.6, solid_capstyle="round")
    arrow(ax, (plus[0], top - 0.2), (plus[0], plus[1] + 0.09), color=GREY, lw=0.6, ms=5)
    ax.text(3.1, top + 0.03, "residual", ha="center", va="bottom", fontsize=6, color=GREY)
    ax.text(2.85, 0.10, "$W_k$: one $C\\times C$ matrix per region, zero-initialised (the layer is the identity at init).  "
...
def skips():
    ...
    def unet(x0, title, shifted):
        ...
        E, D = [], []
        for i, y in enumerate(ys):
            E.append(box(ax, (ex, y), bw, bh, f"enc$_{i}$", fc="#e3ebf5", ec=BLUE, fs=6.5))
            D.append(box(ax, (dx, y), bw, bh, f"dec$_{i}$", fc="#f8e8dc", ec=ORANGE, fs=6.5))
        for i in range(2):
            arrow(ax, (E[i][0], E[i][1] - bh / 2), (E[i + 1][0], E[i + 1][1] + bh / 2), color=BLUE)
            arrow(ax, (D[i + 1][0], D[i + 1][1] + bh / 2), (D[i][0], D[i][1] - bh / 2), color=ORANGE)
        arrow(ax, (E[2][0], E[2][1] - bh / 2), (D[2][0], D[2][1] - bh / 2), color=GREY, rad=0.5)
        ax.text((ex + dx + bw) / 2, 0.36, "bottleneck", ha="center", fontsize=5.5, color=GREY)
        ...
        for i in range(3):
            if shifted:
                ...
                y_src = 1.95 - 0.02 if i == 0 else (E[i - 1][1] - bh / 2 + E[i][1] + bh / 2) / 2
            ...
            ax.text((src[0] + dst[0]) / 2, src[1] + 0.035, lab, ha="center", va="bottom", fontsize=5, color=GREY)
        ax.text(x0 + 1.15, 0.12, title, ha="center", fontsize=7)

    unet(0.15, "standard: post-encoder $\\to$ pre-decoder", shifted=False)
    unet(3.05, "this work: pre-encoder $\\to$ post-upsample (shifted one level)", shifted=True)
    save(fig, "skips")
...
def pipeline():
    ...
    ya = 2.30
    ax.text(0.10, ya + h + 0.05, "A. generator", ha="left", va="bottom", fontsize=6.5, color=BLUE)
    A = row(ya, [(0.10, "training images\n+ detected layouts"),
                 (1.35, "flow matching,\nx-prediction\n(U-Net + RegionPool)"),
                 (2.60, "velocity model\n$\\hat x(x_t, m, t)$"),
                 (3.85, "distil the midpoint-8\nsampler (fixed teacher)")], "#e3ebf5", BLUE)
    box(ax, (5.05, ya), 0.48, h, "flow map\n$u_\\theta$", fc="#f8e8dc", ec=ORANGE, fs=5.8)
    arrow(ax, (A[-1][0] + w / 2, A[-1][1]), (5.05, ya + h / 2), color=BLUE)
    # branch B: the layout prior
    yb = 1.52
    ax.text(0.10, yb + h + 0.05, "B. layout prior", ha="left", va="bottom", fontsize=6.5, color=GREEN)
    B = row(yb, [(0.10, "detected layouts\n(28 landmarks)"),
                 (1.35, "PDM: pose\n$(c_x, c_y, \\log s, \\theta)$ + shape"),
                 (2.60, "PCA on shape\n(99 % variance)"),
                 (3.85, "Gaussian mixture\nover [pose, PCA]")], "#e2f0e7", GREEN)
    box(ax, (5.05, yb), 0.48, h, "prior", fc="#e2f0e7", ec=GREEN, fs=6)
    arrow(ax, (B[-1][0] + w / 2, B[-1][1]), (5.05, yb + h / 2), color=GREEN)
    # inference
    yi = 0.55
    ax.text(0.10, yi + h + 0.05, "C. inference", ha="left", va="bottom", fontsize=6.5, color=ORANGE)
    C = row(yi, [(0.10, "sample a layout\nfrom the prior"),
                 (1.35, "(optional) edit pose,\neyes, mouth"),
                 (2.60, "rasterise hulls\n$\\to$ masks $m$"),
                 (3.85, "1–2 jumps:\n$x_0 + u_\\theta(x_0, m, 0, 1)$")], "#f8e8dc", ORANGE)
    ...
    def elbow(pts, color):
        ax.plot([q[0] for q in pts], [q[1] for q in pts], color=color, lw=0.7, ls=(0, (2, 2)))
        arrow(ax, pts[-2], pts[-1], color=color, lw=0.7, ms=5)

    elbow([(5.29, yb), (5.29, 1.32), (0.95, 1.32), (0.95, yi + h)], GREEN)
    elbow([(5.53, ya + h / 2), (5.57, ya + h / 2), (5.57, 1.22), (4.35, 1.22), (4.35, yi + h)], ORANGE)
    ax.text(2.8, 0.14, "A and B are fitted independently from the same detected layouts.  Only C runs at inference time: no real image is involved.",
            ha="center", fontsize=5.6, color=GREY)
    save(fig, "pipeline")
...
def distill():
    ...
    ax.text(2.8, 2.38, "teacher: midpoint sampler, 8 steps, on the velocity model", ha="center", fontsize=6, color=GREY)
    for p_, lab, dy in ((x0, "$x_0$ (noise)", -0.13), (xh, "$x_{1/2}$", 0.1), (x1, "$x_1$", -0.13)):
        ax.plot(*p_, "o", ms=4.5, color=INK)
        ax.text(p_[0], p_[1] + dy, lab, ha="center", va="top" if dy < 0 else "bottom", fontsize=7)

    def jump(p, q, lab, rad, color, dy):
        arrow(ax, p, q, color=color, lw=1.1, rad=rad, ms=8)
        mx, my = (p[0] + q[0]) / 2, (p[1] + q[1]) / 2
        ax.text(mx, my + dy, lab, ha="center", va="center", fontsize=6.2, color=color,
                bbox={"fc": "white", "ec": "none", "pad": 1})

    # A jump is a straight displacement: chords against the curved teacher path.
    jump(x0, x1, "1 jump:  $x_1 \\approx x_0 + u_\\theta(x_0, 0, 1)$", 0.0, ORANGE, -0.14)
    jump(x0, xh, "2 jumps:  $x_0+\\frac{1}{2}u_\\theta(x_0,0,\\frac{1}{2})$", 0.0, GREEN, -0.16)
    jump(xh, x1, "$x_{1/2}+\\frac{1}{2}u_\\theta(x_{1/2},\\frac{1}{2},1)$", 0.0, GREEN, -0.16)
    ax.text(2.8, 0.30, r"$\mathcal{L}=\mathbb{E}\,\|\,x_t+(s-t)\,u_\theta(x_t,m,t,s)-x_s^{\mathrm{teacher}}\|^2,"
